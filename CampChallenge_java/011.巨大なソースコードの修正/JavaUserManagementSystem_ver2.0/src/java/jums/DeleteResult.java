...
public class DeleteResult extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        // 中身が空っぽだったのですべて作成しました。参考にしたのはUpdateResult.java
        ...
        try{
            ...
            //アクセスルートチェック(acの値が飛んだ時(indexに戻った後でブラウザバックしたなど)に対応)
            ...
            request.setAttribute("deleteData",userData);
            
            request.getRequestDispatcher("/deleteresult.jsp").forward(request, response);
        }catch(Exception e){
            //何らかの理由で失敗したらエラーページにエラー文を渡して表示。想定は不正なアクセスとDBエラー、削除されているデータを削除しようとしたなど
